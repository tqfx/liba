...
int MAIN(int argc, char *argv[]) // NOLINT(misc-definitions-in-headers)
{
    ...
    a_pid_s ctx[2];
    a_pid_init(ctx + 0, A_FLOAT_C(0.01), -10, +10);
    a_pid_init(ctx + 1, A_FLOAT_C(0.01), -10, +10);
    a_pid_off(a_pid_inc(a_pid_pos(ctx + 0, 10)));
    a_pid_off(a_pid_inc(a_pid_pos(ctx + 1, 10)));
    a_pid_kpid(ctx + 0, 10, A_FLOAT_C(0.01), A_FLOAT_C(0.1));
    a_pid_kpid(ctx + 1, 10, A_FLOAT_C(0.01), A_FLOAT_C(0.1));
    a_pid_set_dt(ctx + 0, A_FLOAT_C(0.001));
    a_pid_set_dt(ctx + 1, A_FLOAT_C(0.001));
    ...
    for (a_float_t t = 0; t < A_FLOAT_C(0.4); t += A_FLOAT_C(0.001))
    {
        ...
        buf[1] = *v1;
        buf[2] = *v2;
        a_float_t const *const ptr = a_pid_outp(ctx + 1, set, buf);
