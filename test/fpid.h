...
#define S 1
    ...
    a_fpid_init(ctx + 0, A_FLOAT_C(0.01), 7, me, mec, mkp, mki, mkd, -10, +10);
    {
        static unsigned int idx[A_FPID_IDX(2)];
        static a_float_t val[A_FPID_VAL(2)];
        a_fpid_buff(ctx + 0, idx, val);
        a_fpid_pos(ctx + 0, 10);
    }
    a_fpid_kpid(ctx + 0, 400, 200, A_FLOAT_C(0.005));
    a_pid_set_dt(&ctx[0].pid, A_FLOAT_C(0.001));
    a_fpid_init(ctx + 1, A_FLOAT_C(0.01), 7, me, mec, mkp, mki, mkd, -10, +10);
    {
        static unsigned int idx[A_FPID_IDX(2)];
        static a_float_t val[A_FPID_VAL(2)];
    ...
    a_fpid_kpid(ctx + 1, 400, 200, A_FLOAT_C(0.005));
    ...
    for (a_float_t t = 0; t < A_FLOAT_C(0.2); t += A_FLOAT_C(0.001))
    {
        ...
        buf[1] = *v1;
        buf[2] = *v2;
        a_float_t const *const ptr = a_fpid_outp(ctx + 1, set, buf);
