...
{
    a_real const r = a_point3_dist2(lhs, rhs);
    return a_real_sqrt(r);
}
...
a_real a_point3_tricir2(a_point3 const *p1, a_point3 const *p2, a_point3 const *p3, a_point3 *pc)
{
    ...
    if (A_ABS(d) >= A_REAL_EPS)
    {
        ...
        u = c * (a - b) * d;
        v = a * (c - b) * d;
        pc->x = u * v12.x + v * v13.x;
        pc->y = u * v12.y + v * v13.y;
        pc->z = u * v12.z + v * v13.z;
        r = pc->x * pc->x + pc->y * pc->y + pc->z * pc->z;
        pc->x += p1->x;
        pc->y += p1->y;
        pc->z += p1->z;
    }
                        ...
                        a_point3 *pc)
{
    ...
    if (A_ABS(d) >= A_REAL_EPS)
    {
        a_real const b21 = a_vector3_dot(&v12, &v12);
        a_real const b31 = a_vector3_dot(&v13, &v13);
        a_real const b41 = a_vector3_dot(&v14, &v14);
