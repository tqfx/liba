...
void a_pid_neuro_zero(a_pid_neuro *const ctx)
{
    a_pid_zero(&ctx->pid);
    ctx->ec = 0;
}

void a_pid_neuro_init(a_pid_neuro *const ctx) { a_pid_neuro_zero(ctx); }

void a_pid_neuro_kpid(a_pid_neuro *const ctx, a_float const k, a_float const kp, a_float const ki, a_float const kd)
{
    a_pid_kpid(&ctx->pid, kp, ki, kd);
    ctx->k = k;
}

void a_pid_neuro_wpid(a_pid_neuro *const ctx, a_float const wp, a_float const wi, a_float const wd)
{
    ctx->wp = wp;
    ctx->wi = wi;
    ctx->wd = wd;
...
a_float a_pid_neuro_run(a_pid_neuro *const ctx, a_float const set, a_float const fdb)
{
    a_float const err = set - fdb;
    return a_pid_neuro_run_(ctx, set, fdb, err, err - ctx->pid.err);
}
a_float a_pid_neuro_run_(a_pid_neuro *const ctx, a_float const set, a_float const fdb, a_float const err, a_float const ec)
{
...
a_float a_pid_neuro_inc(a_pid_neuro *const ctx, a_float const set, a_float const fdb)
{
    a_float const err = set - fdb;
    return a_pid_neuro_inc_(ctx, fdb, err, err - ctx->pid.err);
}
a_float a_pid_neuro_inc_(a_pid_neuro *const ctx, a_float const fdb, a_float const err, a_float const ec)
{
