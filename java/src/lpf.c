...
{
    a_lpf_s ctx;
    jbyteArray jctx = (*jenv)->NewByteArray(jenv, sizeof(a_lpf_s));
    (*jenv)->GetByteArrayRegion(jenv, jctx, 0, sizeof(a_lpf_s), (jbyte *)&ctx);
    a_lpf_init(&ctx, A_LPF_GEN(fc, ts));
    (*jenv)->SetByteArrayRegion(jenv, jctx, 0, sizeof(a_lpf_s), (jbyte *)&ctx);
