...
{
    a_hpf_s ctx;
    jbyteArray jctx = (*jenv)->NewByteArray(jenv, sizeof(a_hpf_s));
    (*jenv)->GetByteArrayRegion(jenv, jctx, 0, sizeof(a_hpf_s), (jbyte *)&ctx);
    a_hpf_init(&ctx, A_HPF_GEN(fc, ts));
    (*jenv)->SetByteArrayRegion(jenv, jctx, 0, sizeof(a_hpf_s), (jbyte *)&ctx);
